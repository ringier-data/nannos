...
#: The fields an edit may change: the watch definition. Schedule and delivery have their
#: own owners on a shared job (the trigger policy, each subscriber), and the job type of
#: an existing job does not change — so a draft that proposed them would show the user a
#: change the page then silently does not apply.
_EDITABLE_DRAFT_FIELDS = frozenset(
    {
        "check_tool",
        "check_args",
        "check_args_exprs",
...
def _editing_section(current: ScheduledJobDraft) -> str:  # type: ignore[valid-type]
    ...
    shown = {k: v for k, v in current.model_dump(exclude_none=True, mode="json").items() if k in _EDITABLE_DRAFT_FIELDS}
    return (
        "You are EDITING an existing watch job, not creating one. The job as it stands:\n"
        ...
        "To remove a field, set it to null. When you change cel_expr, give the whole new "
        "expression (the current one with the change folded in), never a fragment. When "
        "you change check_tool, also give its check_args and cel_expr: the current ones "
        "were written for the old tool. Only these fields can change: "
        f"{', '.join(sorted(_EDITABLE_DRAFT_FIELDS))}.\n\n"
    )
...
def _apply_edit(
    ...
    for key, value in proposed.items():
        # check_tool is what the job is about; nulling it leaves nothing to edit.
        if value is None and key in _EDITABLE_DRAFT_FIELDS and key != "check_tool":
            after.pop(key, None)
    changed = draft.model_fields_set & _EDITABLE_DRAFT_FIELDS
    for key in changed:
        after[key] = getattr(draft, key)

    # A new tool returns a different shape: what was written for the old one goes unless
    # the edit restated it — the rule the form applies when the tool is picked by hand.
    if "check_tool" in changed and after.get("check_tool") != before.get("check_tool"):
    ...
    if "sub_agent_id" in changed and "notification_message" not in changed:
        after.pop("notification_message", None)
    if "notification_message" in changed and "sub_agent_id" not in changed:
...
async def generate_job_draft(
    ...
    with attribution_scope(user_sub=current_user.sub, service=SERVICE_CONSOLE):
        ...
        proposed_cel = result.get("cel_expr")
        result = await _repair_cel(
            result, prompt, _generate, data.query, payload=data.result, judge_fallback=current is None
        )
        # On an edit, an expression that could not be repaired is not replaced by a judge
        # of the user's words — "also tell me when it is cancelled" is a change, not a
        # condition. Keeping the old expression would apply the rest of the edit without
        # its point, so the edit is refused and the person refines the expression itself.
        if (
            current is not None
            and isinstance(proposed_cel, str)
            and proposed_cel.strip()
            and not result.get("cel_expr")
            and not result.get("llm_condition")
        ):
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail=(
        ...
        if current is not None:
            # An existing job keeps its type whatever the model says.
            generated.pop("job_type", None)
