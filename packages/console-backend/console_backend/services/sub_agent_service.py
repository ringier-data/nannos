...
def _normalize_thinking_config(
    ...
    Args:
        model: The concrete model alias (e.g., 'claude-sonnet-4.6'), or None when the agent
            inherits the orchestrator's model or is bound to a tier.
        model_tier: The capability tier ('low'/'standard'/'premium') the agent binds to, or
            None. Mutually exclusive with model. A tier resolves to a concrete model at
            runtime, so its thinking capability can't be checked here — the user's choice is
    ...
    """
    # Only a truly model-less agent (no concrete model AND no tier) inherits the
    # orchestrator's model, so the orchestrator owns thinking — drop it. A tier-bound
    # agent resolves to a concrete model at runtime; preserve the user's choice.
    if model is None and model_tier is None:
        return (None, None)
    # Drop thinking only when we have a concrete alias that we positively know doesn't
    # support it. When supported_models is None the capability is unknown, and a tier has
    # no concrete alias to check here — in both cases preserve the user's choice instead of
...
class SubAgentService:
    ...
    async def create_sub_agent(
        ...
        normalized_enable_thinking, normalized_thinking_level = _normalize_thinking_config(
            data.model,
            data.model_tier.value if data.model_tier else None,
