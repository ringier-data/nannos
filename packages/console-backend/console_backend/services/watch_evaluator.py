...
class WatchOutcome:
    ...
    #: What the condition's expression returned, when it returned something other than a
    #: boolean: the items it matched, untruncated. A CEL condition is both the gate and
    #: the filter — the author wrote it to pick out what matters from the response — so
    #: this is what a triggered watch hands on (to the agent, to the notification writer)
    #: in place of the whole response. None for a boolean gate and for a judged watch,
    #: where nothing narrower than ``check_result`` exists. Kept apart from
    #: ``evaluation.extracted``, which is the same value shrunk for storage on the run.
    evidence: Any = None
...
class WatchEvaluator:
    ...
    async def _evaluate_cel(
        ...
        evidence = None if cel.is_boolean else cel.value
