...
async def get_embedding_readiness(
    ...
    """Resolve embedding readiness to ``(status, configured_alias, reason)``.
    ...
    actually run). Fails open (``ready``) when the gateway can't be read, matching the
    retirement checks, so a transient gateway outage doesn't hard-disable the feature.
    """
...
def _chat_tiers_feature(defaults: dict[str, str], registered: set[str] | None) -> FeatureStatus:
    ...
    if missing:
        return FeatureStatus(
            ...
            status="limited",
            detail=detail,
            caveat=f"{' and '.join(missing)} tier not set — sub-agents bound to it run on the "
