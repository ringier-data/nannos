...
def _create_hitl_middleware(platform_tools: list[BaseTool] | None = None) -> ConditionalHumanInTheLoopMiddleware:
    ...
    *platform_tools* are the orchestrator's own static tools (time, presigned-url,
    copy_file, notify_user, ``FinalResponseSchema``). They are dispatched from
    ``DynamicToolDispatchMiddleware.static_tools`` / ToolNode, not from the
    per-user ``tool_registry``, so without them here the gate cannot fetch a
    callable tool — and a call it cannot fetch is deliberately never classified
    (see ``score_tool_risk``), which would silently drop these to a name-only
    score instead of gating them on their real schema.
    """
