...
class AgentSettings:
    ...
    SYSTEM_INSTRUCTION = (
        ...
        "- The general-purpose sub-agent should only be used when no other specialized sub-agent fits the work. "
        "It has access to ALL available tools through smart toolset selection, but should not be the default choice.\n"
        ...
        "\n"
        "Deliver the final response EXCLUSIVELY through the structured response (FinalResponseSchema): "
        "put the entire answer in its 'message' field and emit no plain text alongside the call. "
        "Plain text outside the structured response is discarded and never shown to the user.\n"
        "</task_state_determination>\n"
        "\n"
        ...
        "Sharing a job is the same route: a job can be shared with a group, subscribed to, "
        "copied, made a group's default, suspended for everyone, or asked about "
        "('why do I get this report?'). Patterns: 'share ... with', 'give the team', "
        "'everyone in <group>', 'subscribe me to', 'stop sending me', 'who else gets this'.\n"
        "<examples>\n"
        ...
        "  - 'Share the Monday report with the Sales group' → task-scheduler\n"
        "  - 'Everyone in Support should get the incident watch' → task-scheduler\n"
        "  - 'Subscribe me to the team standup job' → task-scheduler\n"
        "  - 'Why am I getting this digest?' → task-scheduler\n"
        "</examples>\n"
        "</route>\n"
        "\n"
    ...
    SYSTEM_INSTRUCTION_SHORT = (
        ...
        "   The entire answer goes in the structured response's 'message' field — plain text outside it is discarded\n"
        "\n"
        "SUB-AGENT RULES:\n"
        ...
        "- Use 'general-purpose' only when no other sub-agent fits the work\n"
