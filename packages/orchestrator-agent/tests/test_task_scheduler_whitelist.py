...
def test_task_scheduler_whitelist_includes_scheduler_and_playground_tools():
    """Test that task-scheduler has access to scheduler_* and playground_* tools."""
    # Create mock tools
    scheduler_tool = Mock()
    scheduler_tool.name = "scheduler_create_job"

    playground_tool = Mock()
    playground_tool.name = "playground_list_sub_agents"
    ...
    user_config = UserConfig(
        ...
        tools=[scheduler_tool, playground_tool, github_tool],
    )
    ...
    # The whitelist should include scheduler_* and playground_* tools
    ...
    assert "playground_list_sub_agents" in whitelisted_names
...
def test_task_scheduler_whitelist_filters_correctly():
    """Test that task-scheduler whitelist only includes scheduler/playground tools."""
    ...
    # Should include all scheduler_* and playground_* tools
    assert "scheduler_create_job" in whitelisted_names
    assert "scheduler_list_jobs" in whitelisted_names
    assert "scheduler_validate_watch" in whitelisted_names
    assert "playground_list_sub_agents" in whitelisted_names
    # playground_create_sub_agent is intentionally excluded from task-scheduler whitelist
    assert "playground_create_sub_agent" not in whitelisted_names
