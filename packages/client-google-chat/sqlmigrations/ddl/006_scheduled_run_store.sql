...
-- Provenance of scheduled-run notifications delivered to Google Chat.
-- Keyed by the delivered message's thread resource name
-- ({thread name} = "spaces/{space}/threads/{thread}", globally unique — it
-- embeds the space, and the same value arrives verbatim as threadId on reply
-- events, so writer and reader share one id source), so a thread reply under
-- a notification can be correlated back to the scheduled job, run, and
-- sub-agent conversation that produced it. The run's A2A context_id is the
-- sub-agent's own conversation and is forwarded to the orchestrator as
-- structured data only — never as the request contextId.
--
...
create table scheduled_run_store (
    context_key text not null primary key,  -- thread resource name of the delivered message
