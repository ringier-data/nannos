...
export class PgOAuthStateStore {
  ...
  async set(
    ...
    try {
      await this.pool.query(SQL`
        INSERT INTO oauth_state (state, user_id, project_id, code_verifier, expires_at)
        VALUES (${state}, ${userId}, ${projectId}, ${codeVerifier}, ${expiresAt})
  ...
  async get(state: string): Promise<OAuthStateData | null> {
    try {
      const result = await this.pool.query(SQL`
        SELECT user_id, project_id, code_verifier, expires_at
        FROM oauth_state
        WHERE state = ${state}
      `);
   ...
   */
  async consume(state: string): Promise<{ userId: string; projectId: string; codeVerifier: string } | null> {
    try {
      const result = await this.pool.query(SQL`
        DELETE FROM oauth_state
        WHERE state = ${state}
        RETURNING user_id, project_id, code_verifier, expires_at
      `);
