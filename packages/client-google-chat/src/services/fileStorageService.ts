...
export class FileStorageService {
   ...
   * Format: google-chat-client/{userId}/{contextId}/{uploadId}/{filename}
   * - userId: Google Chat user ID
   * - contextId: Thread/conversation identifier
  ...
  private generateS3Key(userId: string, contextId: string, fileName: string): string {
    ...
    return `google-chat-client/${userId}/${sanitizedContextId}/${uploadId}-${sanitizedFileName}`;
  }
