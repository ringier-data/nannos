...
export class OIDCClient {
   ...
   * Note: userId and projectId must be added by the caller
  ...
  ): Promise<Omit<UserAuthToken, 'userId' | 'projectId'>> {
    try {
   ...
   * Note: userId and projectId must be added by the caller
   */
  async refreshAccessToken(refreshToken: string): Promise<Omit<UserAuthToken, 'userId' | 'projectId'>> {
    try {
      const config = await this.getConfiguration();
