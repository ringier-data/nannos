...
describe('CompositeUserAuthService (drain)', () => {
  ...
  test('a user who signed in the old way keeps being served locally', async () => {
    ...
    expect(await service.getTokenForAudience('U1', 'P1', 'agent-console')).toBe('local-token');
  ...
  test('a broker row, a row without a refresh token, and no row all go to the broker', async () => {
    for (const current of [
      { userId: 'U1', projectId: 'P1', oidcSub: 's', authMode: 'broker' as const, createdAt: 0, updatedAt: 0 },
      { userId: 'U1', projectId: 'P1', accessToken: 'at', authMode: 'local' as const, createdAt: 0, updatedAt: 0 },
      null,
    ]) {
      row = current;
  ...
  test('every new sign-in goes through the broker, even for a local user', async () => {
    ...
    expect(await service.getAuthorizationUrl('s', 'P1', 'v')).toBe('broker-url');
    await service.storeAuthState('s', 'U1', 'P1');
