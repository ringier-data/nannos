...
export type CatalogModel = {
     ...
     * Provider
     *
     * `litellm_provider` verbatim from LiteLLM's upstream cost map — an *implementation* tag (`bedrock_converse`), the vocabulary a price list needs. NOT the route: LiteLLM's router resolves calls to a runtime family (`bedrock`) at call time, and that is what our proxy cost logger records on usage. Use `family` for anything behavioural.
     */
    provider?: string | null;
    /**
     * Family
     *
     * The provider route/family this entry resolves to (`bedrock`, `vertex_ai`, …), or null when the tag maps to none. Server-derived (`route_family`) so clients never re-implement the tag→family normalization: show THIS, and drive provider-specific UI off it.
     */
    family?: string | null;
    /**
     * Mode
 ...
 * generator does not.
 */
export type GenerateConditionRequest = {
    /**
     * Query
     *
     * What the condition should do, in natural language — either a fresh description or a refinement of current_cel_expr ('also exclude declined attendees').
     */
    query: string;
    /**
     * Current Cel Expr
     *
     * The expression as it stands, when refining rather than starting fresh.
     */
    current_cel_expr?: string | null;
    /**
     * Current Llm Condition
     *
     * The judged condition as it stands, so a refinement can keep or adjust the split.
     */
    current_llm_condition?: string | null;
    /**
     * Result
     *
     * A real tool response to write against and verify with. Without one the expression is only compile-checked, not evaluated.
     */
    result?: unknown;
    /**
     * Check Tool
     *
     * Name of the check tool, for context only.
     */
 ...
 */
export type GenerateConditionResponse = {
    /**
     * Cel Expr
     *
     * The expression. None when the ask is purely semantic (judge-only).
     */
    cel_expr?: string | null;
    /**
     * Llm Condition
     *
     * The semantic stage, when the ask has one; judged on what cel_expr returns.
     */
 ...
 */
export type JobRunStatus = 'running' | 'success' | 'failed' | 'condition_not_met' | 'interrupted' | 'auth_required';
...
export type McpToolInvokeRequest = {
     ...
     * Server Slug
     *
     * Server the tool belongs to. Used only to look up the tool's risk score — the call itself resolves the tool by name across all servers, so a wrong slug cannot make the call fail.
     */
    server_slug?: string | null;
    /**
     * Acknowledge Risk
     *
     * Confirms the caller accepts that the tool may change data. Required when the tool has no risk score or scores above the read-only threshold.
     */
...
export type OutboundScimEndpointCreate = {
     ...
     * Is Mcp Gateway
     *
     * Whether this endpoint is the MCP gateway (Gatana), enabling server access management for synced groups
     */
    is_mcp_gateway?: boolean;
};
...
export type ProviderConfigCheck = {
    /**
     * Unbillable Deployments
     */
    unbillable_deployments: Array<UnbillableDeployment>;
    /**
     * Orphan Cards
     */
    orphan_cards: Array<OrphanCard>;
    /**
     * Gateway Checked
     *
 ...
 */
export type RateCardRekeyRequest = {
    /**
     * Model Name
     */
    model_name: string;
 ...
 */
export type RunTrigger = 'scheduled' | 'retry' | 'manual' | 'resumed';
...
export type ScheduledJobCreate = {
     ...
     * Sub Agent Id
     *
     * ID of an existing sub-agent to execute. Alternatively a custom automated sub-agent can be provided through sub_agent_parameters, for either job type. Required for job_type='task'; optional for 'watch', where it runs when the condition is met and its reply replaces the notification.
     */
    sub_agent_id?: number | null;
    /**
     ...
     * Check Args Exprs
     *
     * Dynamic arguments: a JSON object mapping argument names to CEL expressions over `now` (current time in the job's timezone) and `prev` (the previous check result). Each is evaluated fresh on every run and its value becomes that argument, merged over check_args (the expression wins per key). This is how a rolling time window reaches a tool argument — e.g. {"start_date": "strftime(now - duration('168h'), '%Y-%m-%d')"}. Never put a literal date in check_args; it goes stale.
     */
    check_args_exprs?: {
        [key: string]: string;
    } | null;
     ...
     * Cel Expr
     *
     * CEL (Common Expression Language) condition over `result` (the tool response), `now` (current time in the job's timezone) and `prev` (the previous check result, null on the first run). Does date math, boolean logic and filtering deterministically. A boolean result gates the trigger directly; any other result triggers when non-empty — return the matching items themselves, since what the expression returns is recorded on the run and handed to the model or agent. jsonpath(result, "$.a[*].b") and eq_ci(a, b) are available for JSONPath extraction and case-insensitive comparison. Composes with llm_condition: the CEL gate runs first and the model judges only what passed it. A watch needs cel_expr, llm_condition, or both.
     */
    cel_expr?: string | null;
    /**
     * Llm Condition
     *
     * Natural language condition judged by a model on each run. Use only for genuinely semantic judgement (tone, intent, 'looks external') — never for arithmetic, counting, time windows or string matching, which belong in cel_expr. Composes with cel_expr: the CEL gate runs first, and the model judges what the expression returned.
     */
    llm_condition?: string | null;
    /**
     * Destroy After Trigger
 ...
 * ScheduledJobDraft
 *
 * A partial scheduled job: every ScheduledJobCreate field, all optional. Fields the generator could not infer are omitted for the caller to fill in.
 */
...
export type ScheduledJobUpdate = {
     ...
     * Check Args Exprs
     *
     * Dynamic arguments: argument name to CEL expression over `now`/`prev`, each resolved on every run and merged over check_args. Null clears them.
     */
    check_args_exprs?: {
        [key: string]: string;
    } | null;
     ...
     * Cel Expr
     *
     * CEL condition over `result`, `now` and `prev`. Null clears it, leaving llm_condition as the whole condition — a watch must keep at least one.
     */
    cel_expr?: string | null;
    /**
     * Llm Condition
 ...
 */
export type UnbillableDeployment = {
    /**
     * Model Name
     *
     * The gateway alias — what callers request and what rate cards key on
     */
    model_name: string;
    /**
     * Runtime Provider
     *
     * Provider family the cost logger will stamp on this deployment's usage. None when the deployment id carries no route prefix and no custom_llm_provider — nothing can bill it.
     */
    runtime_provider?: string | null;
    /**
     * Other Providers
     *
     * Providers that DO have an active card pricing this model name (pattern cards included)
     */
    other_providers?: Array<string>;
    /**
     * Rekey Candidates
     *
     * Subset of other_providers a one-click re-key may be offered for: a card keyed on THIS model name that this alias is not ALSO routed as. Excluded are cards pricing another deployment of the same alias (moving one un-bills live traffic) and pattern cards (see pattern_providers).
     */
    rekey_candidates?: Array<string>;
    /**
     * Pattern Providers
     *
     * Providers whose card only prices this model through a regex on ANOTHER model name. They explain why the model has pricing, but a re-key can't move them: the card header is keyed elsewhere, and moving it would take the rest of that family's pricing along.
     */
    pattern_providers?: Array<string>;
    reason: ReasonEnum;
};
 ...
 */
export type ValidateArgsExprRequest = {
    /**
     * Check Args Exprs
     *
     * Argument name to CEL expression over `now` and `prev`.
     */
    check_args_exprs: {
        [key: string]: string;
    };
     ...
     * Check Args
     *
     * The static arguments, so the response shows the merged result.
     */
    check_args?: {
        [key: string]: unknown;
    } | null;
     ...
     * Prev
     *
     * The previous check result, for expressions that use `prev`.
     */
    prev?: unknown;
};
 ...
 */
export type ValidateArgsExprResponse = {
    /**
     * Valid
     *
     * False when the expression could not be compiled or evaluated.
     */
    valid: boolean;
    /**
     * Error
 ...
 */
export type ValidateConditionRequest = {
    /**
     * Result
     *
     * The tool response to evaluate against — either one returned by /mcp/tools/invoke or a payload the author pasted in.
     */
    result: unknown;
    /**
     * Cel Expr
     *
     * CEL condition over `result`, `now` and `prev`. `now` is the server's current time; pass `prev` to try change-detection conditions.
     */
    cel_expr?: string | null;
    /**
     * Prev
     *
     * The previous check result, for trying CEL conditions that use `prev`.
     */
    prev?: unknown;
    /**
     * Llm Condition
     *
     * A judged condition is not judged here — that needs a model call — but its presence changes what the preview reports: with cel_expr, the gate is still evaluated, and a false gate means the model would never be asked.
     */
    llm_condition?: string | null;
};
 ...
 */
export type ValidateConditionResponse = {
    /**
     * Valid
     *
     * False when the expression could not be parsed.
     */
    valid: boolean;
    /**
     * Error
     ...
     * Condition Met
     *
     * None when not evaluated: invalid expression, or judged by a model.
     */
...
export type RenameConversationApiV1ConversationsConversationIdPatchData = {
...
export type RenameConversationApiV1ConversationsConversationIdPatchErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type RenameConversationApiV1ConversationsConversationIdPatchError = RenameConversationApiV1ConversationsConversationIdPatchErrors[keyof RenameConversationApiV1ConversationsConversationIdPatchErrors];

export type RenameConversationApiV1ConversationsConversationIdPatchResponses = {
    /**
     * Successful Response
     */
    204: void;
...
export type RenameConversationApiV1ConversationsConversationIdPatchResponse = RenameConversationApiV1ConversationsConversationIdPatchResponses[keyof RenameConversationApiV1ConversationsConversationIdPatchResponses];
...
export type InvokeMcpToolApiV1McpToolsInvokePostData = {
...
export type InvokeMcpToolApiV1McpToolsInvokePostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type InvokeMcpToolApiV1McpToolsInvokePostError = InvokeMcpToolApiV1McpToolsInvokePostErrors[keyof InvokeMcpToolApiV1McpToolsInvokePostErrors];

export type InvokeMcpToolApiV1McpToolsInvokePostResponses = {
    /**
     * Successful Response
     */
    200: McpToolInvokeResponse;
};

export type InvokeMcpToolApiV1McpToolsInvokePostResponse = InvokeMcpToolApiV1McpToolsInvokePostResponses[keyof InvokeMcpToolApiV1McpToolsInvokePostResponses];
...
export type ListSecretsApiV1SecretsGetErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type ListSecretsApiV1SecretsGetError = ListSecretsApiV1SecretsGetErrors[keyof ListSecretsApiV1SecretsGetErrors];
...
export type ListPendingApprovalsApiV1SubAgentsPendingGetErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type ListPendingApprovalsApiV1SubAgentsPendingGetError = ListPendingApprovalsApiV1SubAgentsPendingGetErrors[keyof ListPendingApprovalsApiV1SubAgentsPendingGetErrors];
...
export type ProviderConfigCheckApiV1AdminRateCardsProviderConfigGetData = {
    body?: never;
    path?: never;
    query?: never;
    url: '/api/v1/admin/rate-cards/provider-config';
};

export type ProviderConfigCheckApiV1AdminRateCardsProviderConfigGetResponses = {
    /**
     * Successful Response
     */
    200: ProviderConfigCheck;
};

export type ProviderConfigCheckApiV1AdminRateCardsProviderConfigGetResponse = ProviderConfigCheckApiV1AdminRateCardsProviderConfigGetResponses[keyof ProviderConfigCheckApiV1AdminRateCardsProviderConfigGetResponses];

export type RekeyRateCardApiV1AdminRateCardsRekeyPostData = {
...
export type RekeyRateCardApiV1AdminRateCardsRekeyPostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type RekeyRateCardApiV1AdminRateCardsRekeyPostError = RekeyRateCardApiV1AdminRateCardsRekeyPostErrors[keyof RekeyRateCardApiV1AdminRateCardsRekeyPostErrors];

export type RekeyRateCardApiV1AdminRateCardsRekeyPostResponses = {
    /**
     * Response Rekey Rate Card Api V1 Admin Rate Cards Rekey Post
     *
     * Successful Response
     */
    200: {
...
export type RekeyRateCardApiV1AdminRateCardsRekeyPostResponse = RekeyRateCardApiV1AdminRateCardsRekeyPostResponses[keyof RekeyRateCardApiV1AdminRateCardsRekeyPostResponses];
...
export type GenerateJobDraftApiV1SchedulerGenerateJobDraftPostData = {
    body: GenerateJobDraftRequest;
    path?: never;
    query?: never;
    url: '/api/v1/scheduler/generate-job-draft';
};

export type GenerateJobDraftApiV1SchedulerGenerateJobDraftPostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type GenerateJobDraftApiV1SchedulerGenerateJobDraftPostError = GenerateJobDraftApiV1SchedulerGenerateJobDraftPostErrors[keyof GenerateJobDraftApiV1SchedulerGenerateJobDraftPostErrors];

export type GenerateJobDraftApiV1SchedulerGenerateJobDraftPostResponses = {
    /**
     * Successful Response
     */
    200: ScheduledJobDraft;
};

export type GenerateJobDraftApiV1SchedulerGenerateJobDraftPostResponse = GenerateJobDraftApiV1SchedulerGenerateJobDraftPostResponses[keyof GenerateJobDraftApiV1SchedulerGenerateJobDraftPostResponses];

export type GenerateConditionApiV1SchedulerGenerateConditionPostData = {
...
export type GenerateConditionApiV1SchedulerGenerateConditionPostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type GenerateConditionApiV1SchedulerGenerateConditionPostError = GenerateConditionApiV1SchedulerGenerateConditionPostErrors[keyof GenerateConditionApiV1SchedulerGenerateConditionPostErrors];

export type GenerateConditionApiV1SchedulerGenerateConditionPostResponses = {
    /**
     * Successful Response
     */
    200: GenerateConditionResponse;
};

export type GenerateConditionApiV1SchedulerGenerateConditionPostResponse = GenerateConditionApiV1SchedulerGenerateConditionPostResponses[keyof GenerateConditionApiV1SchedulerGenerateConditionPostResponses];

export type ValidateArgsExprApiV1SchedulerValidateArgsExprPostData = {
...
export type ValidateArgsExprApiV1SchedulerValidateArgsExprPostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type ValidateArgsExprApiV1SchedulerValidateArgsExprPostError = ValidateArgsExprApiV1SchedulerValidateArgsExprPostErrors[keyof ValidateArgsExprApiV1SchedulerValidateArgsExprPostErrors];

export type ValidateArgsExprApiV1SchedulerValidateArgsExprPostResponses = {
    /**
     * Successful Response
     */
    200: ValidateArgsExprResponse;
};

export type ValidateArgsExprApiV1SchedulerValidateArgsExprPostResponse = ValidateArgsExprApiV1SchedulerValidateArgsExprPostResponses[keyof ValidateArgsExprApiV1SchedulerValidateArgsExprPostResponses];

export type ValidateConditionApiV1SchedulerValidateConditionPostData = {
...
export type ValidateConditionApiV1SchedulerValidateConditionPostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type ValidateConditionApiV1SchedulerValidateConditionPostError = ValidateConditionApiV1SchedulerValidateConditionPostErrors[keyof ValidateConditionApiV1SchedulerValidateConditionPostErrors];

export type ValidateConditionApiV1SchedulerValidateConditionPostResponses = {
    /**
     * Successful Response
     */
    200: ValidateConditionResponse;
};

export type ValidateConditionApiV1SchedulerValidateConditionPostResponse = ValidateConditionApiV1SchedulerValidateConditionPostResponses[keyof ValidateConditionApiV1SchedulerValidateConditionPostResponses];
...
export type SchedulerListJobsErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type SchedulerListJobsError = SchedulerListJobsErrors[keyof SchedulerListJobsErrors];
...
export type ResumeParkedRunApiV1SchedulerJobsJobIdRunsRunIdResumePostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type ResumeParkedRunApiV1SchedulerJobsJobIdRunsRunIdResumePostError = ResumeParkedRunApiV1SchedulerJobsJobIdRunsRunIdResumePostErrors[keyof ResumeParkedRunApiV1SchedulerJobsJobIdRunsRunIdResumePostErrors];

export type ResumeParkedRunApiV1SchedulerJobsJobIdRunsRunIdResumePostResponses = {
    /**
     * Successful Response
     */
    202: RunNowResponse;
...
export type ResumeParkedRunApiV1SchedulerJobsJobIdRunsRunIdResumePostResponse = ResumeParkedRunApiV1SchedulerJobsJobIdRunsRunIdResumePostResponses[keyof ResumeParkedRunApiV1SchedulerJobsJobIdRunsRunIdResumePostResponses];
...
export type GetRunApiV1SchedulerJobsJobIdRunsRunIdGetData = {
    body?: never;
    path: {
        /**
         * Job Id
    ...
    query?: never;
    url: '/api/v1/scheduler/jobs/{job_id}/runs/{run_id}';
};

export type GetRunApiV1SchedulerJobsJobIdRunsRunIdGetErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type GetRunApiV1SchedulerJobsJobIdRunsRunIdGetError = GetRunApiV1SchedulerJobsJobIdRunsRunIdGetErrors[keyof GetRunApiV1SchedulerJobsJobIdRunsRunIdGetErrors];

export type GetRunApiV1SchedulerJobsJobIdRunsRunIdGetResponses = {
    /**
     * Successful Response
     */
    200: ScheduledJobRun;
...
export type SchedulerListSharedJobsErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type SchedulerListSharedJobsError = SchedulerListSharedJobsErrors[keyof SchedulerListSharedJobsErrors];
...
export type ListChannelsApiV1DeliveryChannelsGetErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type ListChannelsApiV1DeliveryChannelsGetError = ListChannelsApiV1DeliveryChannelsGetErrors[keyof ListChannelsApiV1DeliveryChannelsGetErrors];
...
export type ReportVoiceSessionUsageApiV1VoiceSessionsSessionIdUsagePostData = {
...
export type ReportVoiceSessionUsageApiV1VoiceSessionsSessionIdUsagePostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type ReportVoiceSessionUsageApiV1VoiceSessionsSessionIdUsagePostError = ReportVoiceSessionUsageApiV1VoiceSessionsSessionIdUsagePostErrors[keyof ReportVoiceSessionUsageApiV1VoiceSessionsSessionIdUsagePostErrors];

export type ReportVoiceSessionUsageApiV1VoiceSessionsSessionIdUsagePostResponses = {
    /**
     * Response Report Voice Session Usage Api V1 Voice Sessions  Session Id  Usage Post
     *
     * Successful Response
     */
    201: {
...
export type ReportVoiceSessionUsageApiV1VoiceSessionsSessionIdUsagePostResponse = ReportVoiceSessionUsageApiV1VoiceSessionsSessionIdUsagePostResponses[keyof ReportVoiceSessionUsageApiV1VoiceSessionsSessionIdUsagePostResponses];
