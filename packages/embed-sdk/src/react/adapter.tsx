...
export interface NannosHostAdapter {
  ...
  /**
   * New-message notification routing ONLY: whether the chat surface is on
   * screen (suppresses toasts for the conversation in view) and how to bring
   * it into view when a toast is clicked.
   *
   * v2 note: client-actions (`navigate`/`highlight`/`onApplyResult`) are
   * PROVIDER props now — the adapter no longer carries a second, silently
   * competing path (the old `routing` dual-path footgun is gone), and
   * conversation selection is host code over `useConversations`, not adapter
   * state.
   */
  chatSurface?: {
    /** Is the chat surface currently visible? Default: true. */
    isVisible?: () => boolean;
    /** Bring the chat surface into view (open the panel / navigate to it). */
    bringIntoView?: () => void;
  };
  ...
  /**
   * Notification override. Default: the panel's own in-tree toasts. Provide
...
export interface ResolvedHostAdapter {
  ...
  chatSurface: NonNullable<NannosHostAdapter['chatSurface']>;
...
export function resolveHostAdapter(adapter: NannosHostAdapter, config: NannosConfig): ResolvedHostAdapter {
  ...
  return {
    ...
    links: adapter.links ?? {},
    chatSurface: adapter.chatSurface ?? {},
