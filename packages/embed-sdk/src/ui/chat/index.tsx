...
interface ChatAppWrapperProps {
  ...
  /** Compact single-pane layout for embedded/narrow surfaces (hides the sidebar). */
  compact?: boolean;
}
 ...
 */
export function ChatAppWrapper({ socketPath, customHeaders, playgroundMode, compact }: ChatAppWrapperProps) {
  return (
    <SocketProvider socketPath={socketPath} customHeaders={customHeaders}>
      <ChatProvider playgroundMode={playgroundMode} autoSelectConversation={!compact}>
        <ChatApp compact={compact} />
      </ChatProvider>
    </SocketProvider>
  );
}
