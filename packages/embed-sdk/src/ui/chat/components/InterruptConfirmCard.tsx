...
/** The on-screen client-action tool (Embedded Nannos) — rendered specially. */
const CLIENT_ACTION_TOOL_NAME = 'client_action';
...
/** Risk badge row (shared). */
function RiskBadge({ riskMeta }: { riskMeta: RiskMetadata }) {
  const riskLevel = getRiskLevel(riskMeta.score);
  return (
...
/** Dedicated render for a client-action directive: a human-readable heading plus,
 *  for `apply`, the proposed field→value table (rather than a raw `[object Object]`). */
function ClientActionDetails({ action }: { action: ActionRequest }) {
  const args = (action.args || {}) as Record<string, unknown>;
  const kind = String(args.kind ?? 'apply');
  const targetType = (args.target_type as string) || 'object';
  const values = (args.values as Record<string, unknown>) || {};
  const riskMeta = riskMetaOf(action);

  const heading =
    kind === 'apply'
      ? `Apply changes to ${targetType}`
      : kind === 'navigate'
        ? `Open ${String(args.to ?? '')}`
        : kind === 'highlight'
...
function ActionDetails({ action }: { action: ActionRequest }) {
  if (action.name === CLIENT_ACTION_TOOL_NAME) return <ClientActionDetails action={action} />;
