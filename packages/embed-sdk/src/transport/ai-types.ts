...
export type NannosDataParts = {
  ...
  workplan: { todos: TodoItem[] };
  /** One sub-agent thought; id-reconciled while streaming (`complete` flips on close). Persisted. */
  'agent-thought': { agent: string; text: string; complete: boolean; startedAt: number };
  ...
  activity: { text: string; source?: string; kind?: 'note'; ts: number };
   ...
   *  mode. Persisted. */
  'auth-required': { authUrl?: string; tool?: string; message?: string };
 ...
 * time rides, the way an `activity` part carries `data.ts`. Dev mode is the
 * only reader; nothing on the wire depends on it.
 */
export function textArrival(ts: number): ProviderMetadata {
  return { nannos: { ts } };
}
