import type { NannosStrings } from './keys';

/** The SDK's English defaults — the complete table every override merges over. */
export const en: NannosStrings = {
  'panel.title': 'Assistant',
  'panel.close': 'Close',
  'panel.pin': 'Pin panel',
  'panel.unpin': 'Unpin panel',
  'panel.newChat': 'New chat',
  'panel.history': 'History',
  'panel.settings': 'Settings',

  'composer.placeholder': 'Message Nannos…',
  'composer.placeholderStreaming': 'Reply to steer the running answer…',
  'composer.send': 'Send',
  'composer.stop': 'Stop',
  'composer.attach': 'Attach files',
  'composer.record': 'Record audio',
  'composer.recordStop': 'Stop recording',
  'composer.recordPermission': 'Microphone access was denied. Allow microphone access to record audio.',
  'composer.recordError': 'Could not record audio',
  'composer.readOnly': 'This conversation belongs to another surface and is read-only here.',

  'status.connected': 'Connected',
  'status.connecting': 'Connecting…',
  'status.disconnected': 'Disconnected',
  'status.unauthenticated': 'Not signed in',
  'status.signIn': 'Sign in',
  'status.error': 'Connection error',

  'thread.emptyTitle': 'How can I help?',
  'thread.emptyHint': 'Ask a question, or pick a suggestion to get going.',
  'thread.continueTitle': 'Continue where you left off?',
  'thread.loadOlder': 'Load earlier messages',
  'thread.error': 'Something went wrong',
  'thread.newConversation': 'New conversation',
  'thread.activitySteps': 'Worked through {count} steps',
  'thread.activityStep': 'Worked through 1 step',
  'thread.activityApproved': '{count} approved',
  'thread.activityRejected': '{count} rejected',
  'thread.thinking': 'thinking',
  'thread.pendingOne': '1 approval waiting',
  'thread.pendingMany': '{count} approvals waiting',
  'conversations.title': 'Conversations',
  'conversations.search': 'Search conversations…',
  'conversations.hide': 'Hide conversations',
  'conversations.show': 'Show conversations',
  'conversations.empty': 'No conversations yet.',
  'conversations.noMatches': 'No conversations match your search.',
  'conversations.rename': 'Rename conversation',
  'conversations.renameLabel': 'Conversation name',
  'conversations.renameError': 'Could not rename the conversation',
  'conversations.delete': 'Delete conversation',
  'conversations.deleteTitle': 'Delete conversation?',
  'conversations.deleteBody': '“{title}” will be removed from your history.',
  'conversations.deleteConfirm': 'Delete',
  'conversations.deleteCancel': 'Cancel',
  'conversations.deleteError': 'Could not delete the conversation',

  'context.label': 'Context: {label}',

  'hitl.title': 'Approval needed for {toolName}',
  'hitl.titleCount': '{count} approvals needed',
  'hitl.approve': 'Approve',
  'hitl.approveAll': 'Approve all',
  'hitl.rejectAll': 'Reject all',
  'hitl.reject': 'Reject',
  'hitl.requestChanges': 'Request changes',
  'hitl.reasonPlaceholder': 'Tell the assistant what to change…',
  'hitl.risk': 'Risk: {level} ({percent}%)',
  'hitl.riskCritical': 'Critical',
  'hitl.riskHigh': 'High',
  'hitl.riskMedium': 'Medium',
  'hitl.riskLow': 'Low',
  'hitl.diff.field': 'Field',
  'hitl.diff.current': 'Current',
  'hitl.diff.new': 'New',
  'hitl.clientAction.apply': 'Fills out a form on this page.',
  'hitl.clientAction.highlight': 'Points at something on this page.',
  'hitl.clientAction.navigate': 'Opens another page in this app.',
  'hitl.clientAction.readCurrentPage': 'Reads what is on this page.',

  'applyMode.heading': 'Modes',
  'applyMode.manual': 'Manual',
  'applyMode.allowEdits': 'Allow edits',
  'applyMode.manualHint': 'Nannos asks for approval before it fills in a form',
  'applyMode.allowEditsHint':
    'Nannos will fill in forms without asking.',
  'applyMode.label': 'Change how Nannos fills in forms',

  'auth.title': 'Authorization needed',
  'auth.titleService': 'Authorization needed for {service}',
  'auth.body': 'Nannos needs your permission before it can continue.',
  'auth.bodyTool': 'Nannos needs your permission to use {tool}.',
  'auth.action': 'Authorize',
  'auth.retryHint': 'Complete the authorization in the new window, then continue here.',
  'auth.doneAction': 'Done, continue',
  'auth.retryAction': 'Re-authorize',
  'auth.skip': 'Skip',

  'receipt.approved': 'Approved {subject}',
  'receipt.rejected': 'Rejected {subject}',
  'receipt.changes': 'Changes requested for {subject}',
  'receipt.batch': 'Approved {approved} of {total} actions',
  'receipt.batchRejected': '{rejected} rejected',
  'receipt.authorized': 'Authorized {subject}',
  'receipt.skipped': 'Skipped authorizing {subject}',
  'receipt.authorizedAny': 'Authorization complete',
  'receipt.skippedAny': 'Skipped the authorization',
  'receipt.undecided': '{subject} was never decided',
  'receipt.retried': 'asked Nannos to retry',
  'receipt.risk': '{level} risk',

  'working.title': 'Working…',
  'thinking.title': '{agent} is thinking',

  'feedback.helpful': 'Helpful',
  'feedback.notHelpful': 'Not helpful',
  'feedback.prompt': 'How did the assistant do?',
  'report.title': 'Report an issue',
  'report.description':
    'Describe the problem you encountered. Our team will investigate using the conversation context.',
  'report.placeholder': 'What went wrong?',
  'report.submit': 'Send report',
  'report.cancel': 'Cancel',
  'report.success': 'Issue reported',
  'report.error': 'Could not send the report',

  'attachments.uploading': 'Uploading…',
  'attachments.failed': 'Upload failed',
  'attachments.remove': 'Remove attachment',

  'message.copy': 'Copy',
  'message.copied': 'Copied',
  'message.copyFailed': 'Could not copy',
  'message.download': 'Download {name}',
  'panel.export': 'Export conversation',
  'export.truncated': 'This export may be incomplete: only the messages loaded so far are included.',
  'export.user': 'User',
  'export.assistant': 'Assistant',
  'sendMode.heading': 'While the agent is working',
  'sendMode.steer': 'Steer',
  'sendMode.stopAndSend': 'Stop and send',
  'sendMode.steerHint': 'Your message joins the running answer; nothing is interrupted',
  'sendMode.stopAndSendHint': 'The running answer is stopped first, then your message starts a new one',
  'sendMode.label': 'Change what sending does while the agent is working',
};
