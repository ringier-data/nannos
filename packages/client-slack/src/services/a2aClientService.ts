...
export interface A2ARequest {
  ...
  fileUrls?: A2AFileUrl[]; // Attached files with S3 presigned URLs (preferred)
...
export class A2AClientService {
  ...
  async sendMessage(request: A2ARequest, accessToken: string): Promise<A2AResponse> {
    try {
      this.logger.info(`Sending message to A2A server for user ${request.userId}`);
      this.logger.debug(
        `A2A request details: channelId=${request.channelId}, threadTs=${request.threadTs}, contextId=${request.contextId}`
      );
      ...
      this.logger.debug({ a2aSendParams: sendParams }, `Sending message with params`);
      const response = await client.sendMessage(sendParams);
      this.logger.debug({ rawA2AResponse: response }, `Raw A2A response`);

      this.logger.info(`A2A server responded for user ${request.userId}`);
  ...
  async sendMessageStream(request: A2ARequest, accessToken: string, onEvent: A2AStreamCallback): Promise<A2AResponse> {
    try {
      this.logger.info(`Sending streaming message to A2A server for user ${request.userId}: ${request.text}`);
      this.logger.debug(
      ...
      const errorResponse = this.handleError(error, request.userId);
  ...
  async sendMessageAsync(request: A2ARequest, accessToken: string): Promise<A2AResponse> {
    try {
      this.logger.info(`Sending async message to A2A server for user ${request.userId}`);
      this.logger.debug(
        `A2A async request details: channelId=${request.channelId}, threadTs=${request.threadTs}, contextId=${request.contextId}, webhookUrl=${request.webhookUrl}`
      );
      ...
      this.logger.info(`A2A server accepted async request for user ${request.userId}`);
