...
describe('ThinkingStepsStreamer', () => {
  ...
  test('postNote posts the note as an ordinary thread message, not a task card', async () => {
    const s = new ThinkingStepsStreamer(m.client, baseOpts);
    await s.start();
    await s.postNote('  I will check the budget first.  ');

    expect(m.client.chat.postMessage).toHaveBeenCalledWith({
      channel: 'C1',
      thread_ts: '100.0',
      markdown_text: 'I will check the budget first.',
    });
    expect(taskCards(m.appendCalls).some((c) => c.title.includes('budget'))).toBe(false);
    // The thinking widget is untouched: its ts stays the same.
    expect(s.ts).toBe('111.222');
  });

  test('postNote ignores an empty note and a finished stream', async () => {
    const s = new ThinkingStepsStreamer(m.client, baseOpts);
    await s.start();
    await s.postNote('   ');
    await s.finish();
    await s.postNote('too late');
    expect(m.client.chat.postMessage).not.toHaveBeenCalled();
  });

  test('postNote in degraded mode turns the status message into the note and posts a new status below it', async () => {
    const failingStreamer = {
      append: jest.fn(async () => {
        throw new Error('missing_scope');
      }),
    ...
    const client = {
      chatStream: jest.fn(() => failingStreamer),
      chat: {
        postMessage: jest.fn(async () => ({ ok: true, ts: `fb.${++posted}` })),
        update: jest.fn(async () => ({ ok: true })),
        delete: jest.fn(async () => ({ ok: true })),
      },
    } as unknown as WebClient;
    ...
    const s = new ThinkingStepsStreamer(client, baseOpts);
    await s.start(); // degraded → status message fb.1
    expect(s.ts).toBe('fb.1');
    ...
    // A new status message goes below the note and becomes the one to update.
    expect(client.chat.postMessage).toHaveBeenCalledTimes(2);
    expect(s.ts).toBe('fb.2');

    await s.appendAnswer('final answer');
    await s.finish();
    // The answer replaces the NEW status message, not the note.
    expect((client.chat.update as jest.Mock).mock.calls.at(-1)).toEqual([
      { channel: 'C1', ts: 'fb.2', markdown_text: 'final answer' },
    ]);
  });
});
