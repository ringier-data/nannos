...
describe('BrokerUserAuthService', () => {
  ...
  test.each([
    ...
    await expect(service.getOrchestratorToken('U1', 'T1')).rejects.toBe(failure);
  ...
  test('the sign-in state is stored before the link is handed out', async () => {
    ...
    let stored = false;
    const storing = withSlowStore.storeAuthState('s1', 'U1', 'T1').then(() => (stored = true));
    ...
    await expect(withSlowStore.storeAuthState('s2', 'U1', 'T1')).rejects.toThrow('db down');
  });
});
