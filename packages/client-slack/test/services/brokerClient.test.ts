...
describe('BrokerClient', () => {
  ...
  test('builds the authorize URL with client id, redirect URI and state', () => {
    const url = new URL(broker.authorizeUrl('https://slack.example/api/v1/oauth/callback', 'slack-auth-1-U1'));
    ...
    expect(url.searchParams.get('redirect_uri')).toBe('https://slack.example/api/v1/oauth/callback');
    expect(url.searchParams.get('state')).toBe('slack-auth-1-U1');
  });
  ...
  test('the authorize URL is built from the public URL when there is one', async () => {
    ...
    const url = new URL(publicBroker.authorizeUrl('https://slack.example/api/v1/oauth/callback', 's1'));
