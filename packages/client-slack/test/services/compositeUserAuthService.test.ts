...
describe('CompositeUserAuthService (drain)', () => {
  ...
  test('a user the broker serves is not looked up again on every call', async () => {
    row = { userId: 'U1', teamId: 'T1', oidcSub: 's', authMode: 'broker', createdAt: 0, updatedAt: 0 };

    await service.isUserAuthorized('U1', 'T1');
    await service.getOrchestratorToken('U1', 'T1');
    await service.getTokenForAudience('U1', 'T1', 'agent-console');
  ...
  test('a local user who signs in again is served by the broker from then on', async () => {
    ...
    expect(await service.getOrchestratorToken('U1', 'T1')).toBe('broker-orchestrator-token');
    expect(getToken).toHaveBeenCalledTimes(1);
  });
});
