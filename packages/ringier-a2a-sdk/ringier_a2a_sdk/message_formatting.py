...
Lives in the SDK because the key is A2A wire metadata and every writer already
depends on the SDK — console-backend included, which writes the notify-only
notification itself and must not pull the agent stack in for one dict.
"""
...
FORMATTING_RULES: dict[str, str] = {
    "slack": (
        ...
        "A table is standard Markdown pipe syntax (header row, separator row, one row per "
        "item), preceded by a blank line and nothing else on the line before it. "
