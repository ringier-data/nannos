...
This is the single home of the vocabulary. Every A2A server in the repo — the
orchestrator over HTTP, agent-runner, and the in-process server every local
sub-agent runs behind (``agent_common.a2a.local_server``) — emits the same
messages through these builders, so a client reads one dialect regardless of
which process produced the event. ``orchestrator-agent app/core/a2a_extensions``
...
# orchestrator-agent tests/test_a2a_extensions_conformance.py) — console-backend's
# negotiation header and the embed SDK carry their own copies of the same list.
ALL_EXTENSIONS = [
