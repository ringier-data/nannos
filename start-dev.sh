...
ensure_docker_db() {
    ...
    if docker run --rm -d --name playground-db -p 5432:5432 alloy-infrastructure-agents-test-database:latest; then
        ...
        echo -e "${RED}Make sure Docker is running and the image 'alloy-infrastructure-agents-test-database:latest' exists${NC}"
        exit 1
    fi
}
...
if [ "$BACKEND_ENV" = "local" ]; then
    ...
    update_env_var ".env" "DYNAMODB_USERS_TABLE" "dev-alloy-infrastructure-agents-users"
    update_env_var ".env" "LANGSMITH_PROJECT" "dev-alloy-agent-framework"
    
    # Fetch secrets from AWS SSM
    echo -e "${YELLOW}Fetching secrets from AWS SSM...${NC}"
    
    if ! AZURE_OPENAI_API_KEY=$(aws ssm get-parameter --name /alloy/data-sharing/openai-api-key-chatgpt-4o --output json --with-decryption | jq -r .Parameter.Value); then
    ...
    if ! OIDC_CLIENT_SECRET=$(aws ssm get-parameter --name /alloy/infrastructure-agents/oidc-chat-ui-client-secret --output json --with-decryption | jq -r .Parameter.Value); then
...
if [ "$ORCHESTRATOR_ENV" = "local" ]; then
    ...
    if [ "$BACKEND_ENV" = "local" ]; then        
        ...
        RDS_STACK_NAME="${ENV_PREFIX}-alloy-infrastructure-agents-rds"
        ...
        if [ -n "$PG_HOST" ]; then
            ...
            PG_USER=$(aws ssm get-parameter --name /alloy/infrastructure-agents/rds-docstore-username --output json --with-decryption | jq -r .Parameter.Value)
            PG_PASS=$(aws ssm get-parameter --name /alloy/infrastructure-agents/rds-docstore-password --output json --with-decryption | jq -r .Parameter.Value)
        ...
        update_env_var ".env" "CHECKPOINT_DYNAMODB_TABLE_NAME" "${ENV_PREFIX}-alloy-infrastructure-agents-langgraph-checkpoints"
        update_env_var ".env" "CHECKPOINT_S3_BUCKET_NAME" "${ENV_PREFIX}-alloy-infrastructure-agents-orchestrator-checkpoints"
        update_env_var ".env" "DYNAMODB_USERS_TABLE" "${ENV_PREFIX}-alloy-infrastructure-agents-users"
        update_env_var ".env" "LANGSMITH_PROJECT" "${ENV_PREFIX}-alloy-agent-framework"
    fi
    ...
    if ! OIDC_CLIENT_SECRET=$(aws ssm get-parameter --name /alloy/infrastructure-agents/oidc-orchestrator-client-secret --output json --with-decryption | jq -r .Parameter.Value); then
        echo -e "${RED}Failed to fetch OIDC_CLIENT_SECRET from AWS SSM${NC}"
        exit 1
    fi
    update_env_var ".env" "OIDC_CLIENT_SECRET" "$OIDC_CLIENT_SECRET"
...
VITE_LANGSMITH_PROJECT_ID=b3d6bc99-afe9-486a-847a-091dff103a46
EOF
...
if [ "$BACKEND_ENV" != "local" ]; then
    ...
    if [ -n "$PG_HOST" ]; then
        ...
        # Fetch database credentials
        PG_USER=$(aws ssm get-parameter --name /alloy/infrastructure-agents/rds-service-username --output json --with-decryption | jq -r .Parameter.Value)
        PG_PASS=$(aws ssm get-parameter --name /alloy/infrastructure-agents/rds-service-password --output json --with-decryption | jq -r .Parameter.Value)
