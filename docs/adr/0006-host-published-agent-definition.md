...
| `/.well-known/agent-skills/<name>/SKILL.md` | agentskills.io skill: frontmatter `name` (equals the directory), `description` (1-1024 chars, "what and when"), optional `metadata: {nannos-visibility: public\|private, nannos-inline: true\|false}`; the body is the instructions. |
...
- **No tool list on either side means every tool** (2026-09-10). A bound sub-agent
  ...
  narrows it again.
- `metadata.nannos-inline` on a SKILL.md is **optional** (2026-09-25, ADR-0012):
  `true` makes the skill an inlined skill of the bound sub-agent (`SkillRef.inline`),
  so its body is in the system prompt on every turn. Anything but a boolean is a hard
  error for that skill. It is covered by the SKILL.md digest, so it is part of the
  revision. It does not carry over to agents that reference the published skill. The
  `x-nannos-agent.skills_inline` list #290 introduced is removed.
...
wording bumps `FRAMING_TEMPLATE_VERSION` so bound agents re-sync. Version 2
(ADR-0012) drops the inlined-skill text #290 appended, so the re-sync turns it into
the `inline` flag.
...
- **cockpit**: `AGENT.md` frontmatter accepts optional `organization`, `model-tier`,
  ...
  `organization`, `model_tier`, `thinking_level`. A skill that should be inlined sets
  `metadata.nannos-inline: true` in its own SKILL.md (ADR-0012). `REACT_APP_NANNOS_SUB_AGENT_ID` and
