...
- **Scheduled-run conversation adoption (remote-only)**: agent-runner sends the run task's contextId on **remote** sub-agent dispatches, so `scheduled_job_runs.conversation_id` names a real conversation on the executing server; when a reply under the delivered notification opens an orchestrator conversation (conversation-origin extension), the orchestrator re-validates the job/run server-side under the user's token and seeds `a2a_tracking` so a follow-up delegation resumes that conversation. Local/automated runs are **never** adopted (they checkpoint inside agent-runner on a thread key the orchestrator's in-process delegation paths don't use, and seeding them desynchronizes the HITL checkpoint probe); foundry lacks a persisted session rid. Details in `packages/orchestrator-agent/AGENTS.md`.
