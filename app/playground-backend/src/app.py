...
from middleware import OrchestratorAuth, ProxyHeadersMiddleware
from middleware import SessionMiddleware as CustomSessionMiddleware
...
from service_instances import cleanup_services, initialize_services
from starlette.middleware.sessions import SessionMiddleware
from utils.connection_pool import connection_pool
from utils.cookie_signer import verify_cookie
from utils.socket_errors import (
    SocketError,
    create_error_response,
    create_success_response,
)
from utils.socket_events import SocketEvents
from utils.socketio_auth import require_auth as require_socket_auth
from validators import validate_agent_card, validate_message


logger = configure_logger('chat-inspector')
configure_existing_logger(logging.getLogger('controllers'))
configure_existing_logger(logging.getLogger('routers'))
configure_existing_logger(logging.getLogger('services'))
configure_existing_logger(logging.getLogger('middleware'))
configure_existing_logger(logging.getLogger('utils'))
configure_existing_logger(logging.getLogger('models'))
...
async def handle_connect(sid: str, environ: dict[str, Any]) -> bool:
    ...
    user = await sio.app_instance.state.user_service.get_user(stored_session.user_id)  # type: ignore[attr-defined]
    if not user:
...
async def handle_initialize_client(sid: str, data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    Args:
        ...
        data: Request data containing agent URL
...
async def handle_send_message(sid: str, json_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        if resp and hasattr(resp, 'status_code'):
            ...
            if body:
                log_extra['body_preview'] = body[:1000] if isinstance(body, str) else str(body)[:1000]
