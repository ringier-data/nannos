...
class TestSessionService:
    ...
    async def test_session_ttl_set_correctly(self, session_service, test_user, test_config):
        """Test that session TTL is set correctly."""
        session_id = await session_service.create_session(
            user_id=test_user.id,
            refresh_token='test_refresh_token',
            id_token='test_id_token',
        ...
        # Get the session to check TTL
        session = await session_service.get_session(session_id)
        assert session is not None

        # TTL should be approximately issued_at + session_ttl_seconds
        expected_ttl = int(session.issued_at.timestamp() + test_config.session_ttl_seconds)
        assert abs(session.ttl - expected_ttl) <= 1  # Allow 1 second difference for timing
