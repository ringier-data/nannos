...
async def get_latest_audit_log(
    pg_session: AsyncSession, entity_type: str, entity_id: str, action: Optional[str] = None
):
...
class TestAuditedRepositoryBase:
    ...
    async def test_create_logs_audit(
        self, user_repository: UserRepository, pg_session: AsyncSession, test_user_db: User
    ):
        ...
        result = await pg_session.execute(
            text(
                "SELECT * FROM audit_logs WHERE entity_type = 'user' AND entity_id = :entity_id ORDER BY created_at DESC LIMIT 1"
            ),
            {"entity_id": test_user_db.id},
        )
    ...
    @pytest.mark.asyncio
    async def test_update_logs_audit_with_before_after(
        self, user_repository: UserRepository, pg_session: AsyncSession, test_user_db: User, test_admin_user_db: User
    ):
        ...
        # Verify audit log was created in database
        audit_log = await get_latest_audit_log(pg_session, "user", test_user_db.id)

        assert audit_log is not None
        assert audit_log["actor_sub"] == "admin-user-sub"
        assert audit_log["entity_type"] == "user"
        assert audit_log["entity_id"] == test_user_db.id
    ...
    @pytest.mark.asyncio
    async def test_delete_logs_audit(
        self, user_repository: UserRepository, pg_session: AsyncSession, test_user_db: User
    ):
        ...
        assert audit_log is not None
        assert audit_log["actor_sub"] == test_user_db.sub
        assert audit_log["entity_type"] == "user"
        assert audit_log["entity_id"] == test_user_db.id
...
class TestSubAgentRepositoryAudit:
    ...
    @pytest.mark.asyncio
    async def test_approve_version_logs_audit(
        self,
        ...
        await repo.approve_version(db=pg_session, actor=test_admin_user_db, context=context)
        await pg_session.commit()
    ...
    @pytest.mark.asyncio
    async def test_reject_version_logs_audit(
        self,
        pg_session: AsyncSession,
        sub_agent_repository: SubAgentRepository,
        test_user_db: User,
        ...
        await repo.reject_version(db=pg_session, context=context, actor=test_admin_user_db)
        await pg_session.commit()
    ...
    @pytest.mark.asyncio
    async def test_update_permissions_logs_audit(
        self,
        pg_session: AsyncSession,
        sub_agent_repository: SubAgentRepository,
        test_user_db: User,
    ...
    @pytest.mark.asyncio
    async def test_activate_sub_agent_logs_audit(
        self,
        pg_session: AsyncSession,
        sub_agent_repository: SubAgentRepository,
        test_user_db: User,
    ...
    @pytest.mark.asyncio
    async def test_deactivate_sub_agent_logs_audit(
        self,
        pg_session: AsyncSession,
        sub_agent_repository: SubAgentRepository,
    ...
    @pytest.mark.asyncio
    async def test_create_config_version_logs_audit(
        self,
        pg_session: AsyncSession,
        sub_agent_repository: SubAgentRepository,
        test_user_db: User,
...
class TestSecretsRepositoryAudit:
    ...
    @pytest.mark.asyncio
    async def test_create_secret_logs_audit(
        self,
        pg_session: AsyncSession,
    ...
    @pytest.mark.asyncio
    async def test_delete_secret_logs_audit(
        self,
        pg_session: AsyncSession,
        secrets_repository: SecretsRepository,
        test_user_db: User,
    ...
    @pytest.mark.asyncio
    async def test_update_secret_permissions_logs_audit(
        self,
        pg_session: AsyncSession,
        secrets_repository: SecretsRepository,
        test_user_db: User,
