...
import { modelSupportsThinking, getAvailableThinkingLevels } from '@/config/models';
...
export function ExtendedThinkingConfig({
  ...
  const containerClasses = showAsCard 
    ? "rounded-lg border bg-muted/50 p-4 space-y-4" 
    : "space-y-4";
            ...
            >
              <SelectTrigger id="thinking-level" className="w-full max-w-xs">
                <SelectValue placeholder="Select thinking level">
                  {thinkingLevel && getAvailableThinkingLevels(model).find(opt => opt.value === thinkingLevel)?.label}
                </SelectValue>
              </SelectTrigger>
