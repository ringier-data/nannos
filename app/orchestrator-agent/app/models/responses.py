...
class AgentStreamResponse(BaseModel):
    ...
    def working(cls, message: str, **metadata: Any) -> "AgentStreamResponse":
        ...
        return cls(state=TaskState.working, content=message, metadata=metadata if metadata else None)

    @classmethod
    def completed(cls, content: str, **metadata: Any) -> "AgentStreamResponse":
        """Factory method for creating completed responses."""
        return cls(state=TaskState.completed, content=content, metadata=metadata if metadata else None)

    @classmethod
    def failed(cls, message: str, **metadata: Any) -> "AgentStreamResponse":
        """Factory method for creating failed responses."""
        return cls(state=TaskState.failed, content=message, metadata=metadata if metadata else None)

    @classmethod
    def input_required(
        cls, message: str, pending_nodes: Optional[List[str]] = None, **metadata: Any
    ) -> "AgentStreamResponse":
