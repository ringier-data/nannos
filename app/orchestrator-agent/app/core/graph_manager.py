...
class GraphManager:
    ...
    def get_config_signature(self, tools: Optional[list[BaseTool]], subagents: Optional[list[CompiledSubAgent]]) -> str:
    ...
    def create_and_cache_graph(
        self, config_sig: str, tools: Optional[list[BaseTool]], subagents: Optional[list[CompiledSubAgent]]
    ) -> CompiledStateGraph:
        ...
        """
        logger.info(f"Creating new graph for config (tools: {len(tools or [])}, subagents: {len(subagents or [])})")
        ...
        compiled_graph = create_deep_agent(
            ...
            response_format=ToolStrategy(
                FinalResponseSchema
            ),  # Request structured output for task status determination
        )
    ...
    def get_or_create_graph(
        self, tools: Optional[list[BaseTool]], subagents: Optional[list[CompiledSubAgent]]
    ) -> CompiledStateGraph:
