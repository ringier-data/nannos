...
from ..handlers import should_retry, handle_auth_error
from ..subagents import A2ATaskTrackingMiddleware
from ..middleware import TodoStatusMiddleware
from ..middleware import AuthErrorDetectionMiddleware
from ..models import UserConfig, AgentSettings
from ..models import AgentStreamResponse
from ..models import AgentFrameworkAuthError
...
class OrchestratorDeepAgent:
    ...
    async def get_token(self, user_id: str) -> SecretStr:
        """Retrieve personal access token for the user."""
        logger.debug(f"Retrieving token for user_id: {user_id}")
        # TODO: Implement token retrieval logic
        if False:
            raise AgentFrameworkAuthError("User is unauthenticated")
        return SecretStr("dummy-token")

    async def get_config(self, user_id, use_cache=True) -> UserConfig:
        """Get configuration for the orchestrator deep agent."""
        logger.debug(f"Getting config for user_id: {user_id}")
        token = await self.get_token(user_id)

        # TODO: implement actual config retrieval based on token
        sub_agents = await self.agent_discovery_service.discover_agents(token, self.a2a_middleware)
        tools = await self.tool_discovery_service.discover_tools(token)
        logger.debug(f"Discovered {len(sub_agents)} sub-agents: {[agent['name'] for agent in sub_agents]}")
        
        config = UserConfig(
            whoami=user_id,
            token=SecretStr("dummy-token"),
            name="John",
            last_name="Doe",
            language="en",
        ...
        logger.debug(f"Created config with {len(config.sub_agents) if config.sub_agents else 0} sub_agents")
        return config

    async def get_or_create_graph(self, user_id: str, force_refresh: bool = False) -> CompiledStateGraph:
        ...
        Args:
            user_id: User identifier for fetching configuration
            force_refresh: If True, clears cache and re-discovers agents
            
        Returns:
            Tuple of (compiled_graph, config_signature)
        """
        logger.debug(f"Getting or creating graph for user_id: {user_id}, force_refresh: {force_refresh}")
        ...
        # Get user config (includes discovering tools and subagents)
        ...
        compiled_graph = self.graph_manager.get_or_create_graph(
            ...
            subagents=user_config.sub_agents
        )
        
        return compiled_graph

    async def stream(self, query: str, user_id: str, context_id: str, resume: Any = None) -> AsyncIterable[AgentStreamResponse]:
        """
        Stream agent responses using standard A2A protocol states with runtime config injection.
        
        ZERO-TRUST ARCHITECTURE:
        - user_id: Verified user identifier from JWT (used for graph selection)
        ...
        Args:
            ...
            user_id: Verified user identifier from JWT 'sub' claim (for graph selection)
        ...
        Examples:
            ...
            async for response in agent.stream("Hello", "user-sub-123", "conv-456"):
                print(response.content)
            
            # Resume from interrupt
            async for response in agent.stream("I've authorized", "user-sub-123", "conv-456", resume="auth token"):
                print(response.content)
        """
        logger.debug("========== STREAM START (Zero-Trust Graph Architecture) ==========")
        logger.debug(f"Query: {query}")
        logger.debug(f"User ID (verified): {user_id}")
        logger.debug(f"Context ID (conversation): {context_id}")
        
        try:
            # ZERO-TRUST: Get or create graph using VERIFIED user_id from JWT
            ...
            graph = await self.get_or_create_graph(user_id)
        ...
        if resume is not None:
            ...
            logger.info(f"Normal input data: {input_data}")
        ...
        try:
            ...
            if hasattr(final_state, 'interrupts') and final_state.interrupts:
                ...
                if task_state == TaskState.auth_required:
                    logger.info(f"[ORCHESTRATOR] Found auth_required interrupt in final state: {final_state.interrupts[-1].value}")
                    ...
                    yield AgentStreamResponse(
                        state=task_state,
                        content=final_state.interrupts[-1].value.get("message", "Process interrupted. Human intervention required."),
            ...
            if hasattr(final_state, 'next') and final_state.next:
                logger.warning(
                    f"graph in final state but no interrupt: {final_state}"
                )
            ...
            # For simplicity, we re-discover agents and clear caches on any unexpected error
            # Get user config (includes discovering tools and subagents)
            logger.debug(f"Handling unexpected exception during streaming: {e}")
            user_config = await self.get_config(user_id)
            config_sig = self.graph_manager.get_config_signature(
                user_config.tools,
                user_config.sub_agents
            )
            if self.graph_manager.signature != config_sig:
                logger.info("Configuration has changed during streaming. Invalidating caches.")
                self.graph_manager.clear_cache()
