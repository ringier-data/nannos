...
class AuthErrorDetectionMiddleware(AgentMiddleware[AuthErrorState, ContextT]):
    ...
    async def awrap_tool_call(
        ...
        tool_name = request.tool_call.get("name", "")
        logger.info(f"[AUTH MIDDLEWARE awrap_tool_call] Intercepting async {tool_name} tool for auth error detection (BEFORE retry middleware)")
        ...
        try:
            ...
            if isinstance(result, ToolMessage):
                ...
                if (hasattr(result, 'update') and result.update and 
                    'messages' in result.update and result.update['messages']):
                    ...
                    if isinstance(last_msg, ToolMessage):
                        ...
                        # If no A2A auth requirement, fall back to content-based detection
                        if not auth_metadata:
                            auth_metadata = self._detect_auth_error(last_msg.content if isinstance(last_msg.content, str) else "")
    ...
    def _check_a2a_auth_metadata(self, a2a_metadata: Dict[str, Any] | None, subagent_name: str) -> Dict[str, Any] | None:
    ...
    def _detect_auth_error(self, content: str) -> Dict[str, Any] | None:
        ...
        try:
            ...
            if (isinstance(content_dict, dict) and 
                content_dict.get("errorCode") == "need-credentials"):
