...
class TestOktaAuthMiddleware:
    ...
    def test_middleware_initialization(self):
        ...
        assert middleware.okta_domain == 'rcplus.okta.com'
        assert middleware.client_id is None
        assert middleware.audience == 'api://default'
        assert middleware.issuer == 'https://rcplus.okta.com/oauth2/default'
    ...
    def test_middleware_initialization_with_custom_values(self):
        ...
        middleware = OktaAuthMiddleware(
            app,
            okta_domain='custom.okta.com',
            client_id='test-client-id',
            audience='custom-audience'
        )
        
        assert middleware.okta_domain == 'custom.okta.com'
        assert middleware.client_id == 'test-client-id'
        assert middleware.audience == 'custom-audience'
...
class TestOktaAuthMiddlewareDispatch:
    ...
    async def test_mock_okta_enabled(self):
        """Test authentication with MOCK_OKTA enabled."""
        app = Mock()
        middleware = OktaAuthMiddleware(app, client_id='test-client')
        ...
        # Mock MOCK_OKTA environment
        with patch.dict('os.environ', {'MOCK_OKTA': 'true'}):
            expected_response = Response()
            call_next = AsyncMock(return_value=expected_response)
            
            response = await middleware.dispatch(request, call_next)
            
            # Should pass through with mocked user
            assert response == expected_response
            assert hasattr(request.state, 'user')
            assert request.state.user['sub'] == 'mock-user-id'
            assert request.state.user['email'] == 'mock-user@example.com'  # Fixed expected value
